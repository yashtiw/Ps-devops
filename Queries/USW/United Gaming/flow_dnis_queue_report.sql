DROP PROCEDURE IF EXISTS `rep_88`.`flow_dnis_queue_report`;

CREATE PROCEDURE `rep_88`.`flow_dnis_queue_report`(
...
BEGIN
    SET @sql = CONCAT(
        ...
        FROM
            rep_88.dnis_queue_summary dqs
            LEFT JOIN rep_88.queue q ON dqs.queue_id = q.id
            LEFT JOIN rep_88.flow f ON dqs.flow_id = f.id
        WHERE
