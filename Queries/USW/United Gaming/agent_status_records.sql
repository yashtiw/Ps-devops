DROP PROCEDURE IF EXISTS `rep_88`.`agent_status_records`;

CREATE PROCEDURE `rep_88`.`agent_status_records`(
...
BEGIN
    SET @sql = CONCAT(
        ...
        FROM
            rep_88.user_status_record usr
            LEFT JOIN rep_88.user u ON usr.user_id = u.id
            LEFT JOIN rep_88.presence p ON usr.sub_status_id = p.id
        WHERE
