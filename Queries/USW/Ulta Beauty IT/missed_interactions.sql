DROP PROCEDURE IF EXISTS `rep_110`.`missed_interactions`;


CREATE PROCEDURE `rep_110`.`missed_interactions`(
...
BEGIN
    SET @sql = CONCAT(
        ...
        FROM
            rep_110.missed_interactions mi
            LEFT JOIN rep_110.user u ON mi.user_id = u.id
            LEFT JOIN rep_110.flow f ON mi.flow_id = f.id
            LEFT JOIN rep_110.queue q ON mi.queue_id = q.id
        WHERE
