DROP PROCEDURE IF EXISTS `rep_94`.`agent_session_records`;

CREATE PROCEDURE `rep_94`.`agent_session_records`(
...
BEGIN
      ...
      GROUP_CONCAT(DISTINCT
        CONCAT(
          'COALESCE((SELECT SEC_TO_TIME(ROUND(SUM(uss2.status_duration) / 1000)) 
            FROM rep_94.user_substatus_summary uss2 
            JOIN rep_94.presence p ON uss2.sub_status_id = p.id 
    ...
    SET @sql = CONCAT(
        ...
        FROM
            rep_94.user_session_summary uss
            LEFT JOIN rep_94.user u ON uss.user_id = u.id
        WHERE
