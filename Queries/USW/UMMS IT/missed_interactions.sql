DROP PROCEDURE IF EXISTS `rep_94`.`missed_interactions`;


CREATE PROCEDURE `rep_94`.`missed_interactions`(
...
BEGIN
    SET @sql = CONCAT(
        ...
        FROM
            rep_94.missed_interactions mi
            LEFT JOIN rep_94.user u ON mi.user_id = u.id
            LEFT JOIN rep_94.flow f ON mi.flow_id = f.id
            LEFT JOIN rep_94.queue q ON mi.queue_id = q.id
        WHERE
