DROP PROCEDURE IF EXISTS `rep_94`.`contact_history_records`;


CREATE PROCEDURE `rep_94`.`contact_history_records`(
...
BEGIN
    SET @sql = CONCAT(
        'SELECT
            ...
            (SELECT q.name FROM rep_94.queue q WHERE q.id = chr.entry_queue_id) AS entry_queue_name,
            (SELECT q.name FROM rep_94.queue q WHERE q.id = chr.exit_queue_id) AS exit_queue_name,
            ...
            (CASE
                WHEN (chr.transfer_party_type = ''Queue'') THEN (
                    SELECT q.name FROM rep_94.queue q WHERE q.queue_flow_id = chr.transfer_party
                )
                WHEN (chr.transfer_party_type = ''Agent'') THEN CONVERT((
                    SELECT u.name FROM rep_94.user u WHERE u.id = chr.transfer_party
                ) USING utf8mb4)
        ...
        FROM
            rep_94.contact_history_record chr
            LEFT JOIN rep_94.flow f ON chr.flow_id = f.id
            LEFT JOIN rep_94.user u ON chr.user_id = u.id
        WHERE
