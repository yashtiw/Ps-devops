DROP PROCEDURE IF EXISTS `rep_94`.`agent_status_records`;

CREATE PROCEDURE `rep_94`.`agent_status_records`(
...
BEGIN
    SET @sql = CONCAT(
        ...
        FROM
            rep_94.user_status_record usr
            LEFT JOIN rep_94.user u ON usr.user_id = u.id
            LEFT JOIN rep_94.presence p ON usr.sub_status_id = p.id
        WHERE
